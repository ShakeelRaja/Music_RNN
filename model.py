...
class Model(object):
    ...
    def __init__(self, config, training=False):
        ...
        #Reduce output by 1
        logits = tf.reshape(logits_concat, [self.time_batch_len, -1, output_dim]) # make one less in output
...
class NottinghamModel(Model):
    ...
    def calculate_probs(self, logits):
        ...
        for t in range(self.time_batch_len):
            melody_softmax = tf.nn.softmax(logits[t, :, :preprocess.Melody_Range])
            harmony_softmax = tf.nn.softmax(logits[t, :, preprocess.Melody_Range:])
