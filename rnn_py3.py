...
from model_new import Model, NottinghamModel
...
#    assert data.shape[0][0] == num_time_steps * time_batch_len 

    # split sequences into time batches
    tb_data = np.split(data, num_time_steps, axis=1)
    tb_targets = np.split(targets, num_time_steps, axis=1)
...
#                continue
        ...
        dataset_data =  [batch_data(b, n) for n, b in batches.items()]
...
runs = list(list(zip(grid_search, x)) for x in itertools.product(*grid_search.values()))
...
#
    ...
    with tf.Graph().as_default(), tf.Session() as session:
        ...
        #################################
#        batchesx = data_split["train"]["data"]
#        training = True
